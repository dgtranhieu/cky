...
void setLineWrap( bool wrap ) {
    lineWrap = wrap;
    writeLog( LOG_LOGGER, "Line wrapping %s", wrap ? "enabled" : "disabled" );
}
...
void loadConfig( const char* config ) {
    ...
    if( fd == -1 ) {
        // Write error message
        writeLog( LOG_LOGGER, "Unable to open config file: '%s'", config );
...
;   // set to the maximum possible path size
    ...
    const char* command;
        ...
        if( !system( "which gaddr2line > /dev/null 2>&1" ) ) {
            ...
            if( path_length <= 0 ) {
                writeLog( LOG_LOGGER, "Unable to get execution path. Defaulting to standard backtrace." );
                error = true;
            }
            exe_path[path_length] = 0;
        } else {
            writeLog( LOG_LOGGER, "Function 'gaddr2line' unavailable. Defaulting to standard backtrace. Please install package 'binutils' for better stacktrace output." );
            error = true;
        }
    #else
        ...
        if( !system( "which addr2line > /dev/null 2>&1" ) ) {
            ...
            writeLog( LOG_LOGGER, "Function 'addr2line' unavailable. Defaulting to standard backtrace. Please install package 'binutils' for better stacktrace output." );
            error = true;
        }
    #endif
    ...
    for( int i = 0; i < array_size; i++ ) {
        ...
        if( line == NULL ) {
            writeLog( LOG_LOGGER, "Failed to execute command: '%s'. Defaulting to standard backtrace.", command );
            for( int i = 0; i < array_size; i ++ ) {
                free( backtrace_strings[i] );
            }
            free( backtrace_strings );
        ...
        if( fgets( backtrace_strings[i] , 255, line ) == NULL ) {
            writeLog( LOG_LOGGER, "Failed to get pretty backtrace strings. Defaulting to standard backtrace." );
            for( int i = 0; i < array_size; i ++ ) {
                free( backtrace_strings[i] );
            }
            free( backtrace_strings );
    ...
    if( !address_evaluation_successful ) {
        writeLog( LOG_LOGGER, "Command '%s' failed to evaluate addresses. Defaulting to standard backtrace.", command );
        for( int i = 0; i < array_size; i ++ ) {
            free( backtrace_strings[i] );
        }
        free( backtrace_strings );
...
_Logger const Logger = { writeLog, writeStackTrace, setLogDebugLevel, setLogFile, setLogSilentMode, setLineWrap, flushLog, loadConfig };