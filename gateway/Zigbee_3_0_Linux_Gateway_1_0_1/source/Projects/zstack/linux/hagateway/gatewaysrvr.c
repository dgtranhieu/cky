...
static void processDevPxhoangEnrollReq( int connection, DevPxhoangEnrollRsp *pWindowCoveringReq );
static ZStatus_t sendDevPxhoangEnrollReq(DevPxhoangEnrollRsp *pPxhoangEnrollReq);

static void processDevPxhoangStartWarningReq( int connection, DevPxhoangStartWarningReq *pPxhoangStartWarningReq );
static ZStatus_t sendDevPxhoangStartWarningReq(DevPxhoangStartWarningReq *pPxhoangStartWarningReq);

static void processDevPxhoangSquawkReq( int connection, DevPxhoangSquawkReq *pPxhoangSquawkReq );
static ZStatus_t sendDevPxhoangSquawkReq(DevPxhoangSquawkReq *pPxhoangSquawkReq);
  ...
  if ( (subSys & RPC_SUBSYSTEM_MASK) == Z_STACK_GW_SYS_ID_T__RPC_SYS_PB_GW )
  {
    switch ( cmdId )
    {
      ...
      case GW_CMD_ID_T__DEV_WINDOW_COVERING_ACTION_REQ:
        ...
        case GW_CMD_ID_T__DEV_PXHOANG_ENROLL_REQ:
            pNativeMsg = dev_pxhoang_enroll_rsp__unpack( NULL, len, pData );
            if ( pNativeMsg )
            {
                printf("pxhoang: %s - unpacked OK\n", __FUNCTION__);
                processDevPxhoangEnrollReq( connection, (DevPxhoangEnrollRsp *)pNativeMsg );
                dev_pxhoang_enroll_rsp__free_unpacked( pNativeMsg, NULL );
            }
            printf("pxhoang: %s - GW_CMD_ID_T__DEV_PXHOANG_ENROLL_REQ\n", __FUNCTION__);
            break;

        case GW_CMD_ID_T__DEV_PXHOANG_START_WARNING_REQ:
            pNativeMsg = dev_pxhoang_start_warning_req__unpack( NULL, len, pData );
            if ( pNativeMsg )
            {
                printf("pxhoang: %s - unpacked OK\n", __FUNCTION__);
                processDevPxhoangStartWarningReq( connection, (DevPxhoangStartWarningReq *)pNativeMsg );
                dev_pxhoang_start_warning_req__free_unpacked( pNativeMsg, NULL );
            }
            printf("pxhoang: %s - GW_CMD_ID_T__DEV_PXHOANG_START_WARNING_REQ\n", __FUNCTION__);
            break;

        case GW_CMD_ID_T__DEV_PXHOANG_SQUAWK_REQ:
            pNativeMsg = dev_pxhoang_squawk_req__unpack( NULL, len, pData );
            if ( pNativeMsg )
            {
                printf("pxhoang: %s - unpacked OK\n", __FUNCTION__);
                processDevPxhoangSquawkReq( connection, (DevPxhoangSquawkReq *)pNativeMsg );
                dev_pxhoang_squawk_req__free_unpacked( pNativeMsg, NULL );
            }
            printf("pxhoang: %s - GW_CMD_ID_T__DEV_PXHOANG_SQUAWK_REQ\n", __FUNCTION__);
            break;
 ...
 * pxhoang: IAS Enroll
 *------------------------------------------------------------------------------------------------*/
static void processDevPxhoangEnrollReq( int connection, DevPxhoangEnrollRsp *pPxhoangEnrollReq )
{
    printf("pxhoang: %s - Get called..\n", __FUNCTION__);
    GwZigbeeGenericCnf genericCnf = GW_ZIGBEE_GENERIC_CNF__INIT;
    ZStatus_t status = ZFailure;

    status = sendDevPxhoangEnrollReq(pPxhoangEnrollReq);

    switch (status)
    {
        case ZSuccess:
            printf("pxhoang: %s - Success\n", __FUNCTION__);
            genericCnf.status = GW_STATUS_T__STATUS_SUCCESS;
            break;

        case ZInvalidParameter:
            printf("pxhoang: %s - Failure = %d\n", __FUNCTION__, status);
            genericCnf.status = GW_STATUS_T__STATUS_INVALID_PARAMETER;
            break;
...
static ZStatus_t sendDevPxhoangEnrollReq(DevPxhoangEnrollRsp *pPxhoangEnrollReq)
{
    ...
    afAddrType_t dstAddr;
    if (!gwConvertAddrPbToAfRsp( pPxhoangEnrollReq->dstaddress, &dstAddr))
        return ZInvalidParameter;

    uint8 srcEp             = srvParseGwEndpointId(pPxhoangEnrollReq->dstaddress);
    uint8 responseCode      = pPxhoangEnrollReq->enrollmentresponsecode;
    uint8 zoneID            = pPxhoangEnrollReq->zoneid;
    uint8 seqNum            = zcl_TransID;
    uint8 disableDefaultRsp = TRUE;

    return zclSS_IAS_Send_ZoneStatusEnrollResponseCmd(srcEp, &dstAddr, responseCode, zoneID, disableDefaultRsp, seqNum);
}


/**-------------------------------------------------------------------------------------------------
 * pxhoang: IAS Start warning
 *------------------------------------------------------------------------------------------------*/
static void processDevPxhoangStartWarningReq( int connection, DevPxhoangStartWarningReq *pPxhoangStartWarningReq )
{
    printf("pxhoang: %s - Get called..\n", __FUNCTION__);
    ...
    status = sendDevPxhoangStartWarningReq(pPxhoangStartWarningReq);
...
static ZStatus_t sendDevPxhoangStartWarningReq(DevPxhoangStartWarningReq *pPxhoangStartWarningReq)
{
    printf("pxhoang: %s - Get called..\n", __FUNCTION__);
    bool disableDefaultRsp;
    uint8 addrModeMask = ADDR_MODE_UNICAST + ADDR_MODE_GROUPCAST + ADDR_MODE_BROADCAST;

    afAddrType_t dstAddr;
    if (!gwConvertAddrPbToAfReq( pPxhoangStartWarningReq->dstaddress, &dstAddr, &disableDefaultRsp, addrModeMask))
        return ZInvalidParameter;

    zclCmdSSWDStartWarningPayload_t cmd;
    uint8 srcEp = srvParseGwEndpointId(pPxhoangStartWarningReq->dstaddress);
    cmd.warningmessage.warningbyte = (uint8_t)pPxhoangStartWarningReq->warningmessage;
    cmd.warningDuration = (uint16_t)pPxhoangStartWarningReq->warningduration;
    cmd.strobeDutyCycle =  (uint8_t)pPxhoangStartWarningReq->strobedutycycle;
    cmd.strobeLevel = (uint8_t)pPxhoangStartWarningReq->strobelevel;

    return zclSS_Send_IAS_WD_StartWarningCmd( srcEp, &dstAddr, &cmd, disableDefaultRsp, zcl_TransID );
}
...
static void processDevPxhoangSquawkReq( int connection, DevPxhoangSquawkReq *pPxhoangSquawkReq )
{
    ...
    status = sendDevPxhoangSquawkReq(pPxhoangSquawkReq);
...
static ZStatus_t sendDevPxhoangSquawkReq(DevPxhoangSquawkReq *pPxhoangSquawkReq)
{
    ...
    afAddrType_t dstAddr;
    if (!gwConvertAddrPbToAfReq( pPxhoangSquawkReq->dstaddress, &dstAddr, &disableDefaultRsp, addrModeMask))
        return ZInvalidParameter;

    zclCmdSSWDSquawkPayload_t cmd;
    uint8 srcEp = srvParseGwEndpointId(pPxhoangSquawkReq->dstaddress);
    cmd.squawkbyte = (uint8_t)pPxhoangSquawkReq->payload;
    return  zclSS_Send_IAS_WD_SquawkCmd( srcEp, &dstAddr, &cmd, disableDefaultRsp, zcl_TransID );
}