...
#define ZS_APP_CNF_COMMISSIONING_NOTI_CBID  0x0080
...
extern pthread_mutex_t zstack_mutex;
extern pthread_mutexattr_t zstack_mutex_attr;
extern bool zstack_mutex_valid;
...
extern void ZMacSetJammerParameters( uint16 contEvents, int8 highNoiseLevel,
                                     uint16 detectPeriod );
extern void GetDevNwkInfo( nwkIB_t* NIB );
static void processSetDevNwkInfo( int connection, UtilSetDevNwkInfoReq* pMsg );
static void processGpAddressConflict( int connection, AFAddr* pMsg );
...
void *APPCNFCommissioningNotiCB( void *pStr );
...
static void processNwkCommissioningNoti( uint8 status, uint8 commMode, uint8 remainingCommMode );
static void processAfIncomingMsgInd( afIncomingMSGPacket_t *pkt );
static void processGpSecReqMsgInd( gp_SecReq_t *pkt );
static void processGpDataIndMsgInd( gp_DataInd_t *pkt );
...
static void processAppCnfStartCommissioningReq( int connection,
                                         AppCnfStartCommissioningReq *pMsg );
static void processAppCnfAddInstallCodeReq( int connection,
                                         AppCnfAddInstallCodeReq*pMsg );
static void processZclActionReq( int connection,
										 ZclActionReq *pMsg );
...
static void SendDevNwkInfo( nwkIB_t* pNIB );
void sendDeviceAnnceShadowGw( uint16 srcAddr, ZDO_DeviceAnnce_t *pDevAnn );
...
void znpInit( uint8 triggerStartEvent )
{
  ...
  //Trust Center Link Key Requirement settings
  ZDP_AppCnfSetTCRequireKeyExchangeReq((bool)zgDefaultTCRequireKeyExchange);

  //Install Code Requirement settings
  ZDP_AppCnfSetJoinUsesICKeyReq((bool)zgDefaultJoinUsesICKey);

  //Set Remote End Device Timeout
  ZDP_AppCnfSetRemoteEndDeviceTimeoutReq((uint8)zgDefaultRemoteEndDeviceTimeout);

  //Channel settings
  uint32 no_channel = 0x00000000;
  ZDP_AppCnfSetChannelReq(FALSE, no_channel);
  ZDP_AppCnfSetChannelReq(TRUE, zgDefaultChannelList);

  //PANID settings
  osal_nv_write( ZCD_NV_PANID, 0, sizeof ( zgConfigPANID ), &zgConfigPANID );
  //Logical device type settings (should be ZC)
...
uint16 zspbProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
  	//Processing a Af Incoming Message type packet
    afIncomingMSGPacket_t *MSGpkt = (afIncomingMSGPacket_t *) osal_msg_receive( zspbTaskID );
    ...
    if ( MSGpkt )
    {
      switch ( MSGpkt->hdr.event )
      {
		...
		case APP_CNF_COMM_NOTIFICATION:
		  {
		  	AppCnfCommNotiPacket_t *appCnfMsg = ((AppCnfCommNotiPacket_t *) MSGpkt);
		  	//Use the correct packet type to parse the message
  			//If the newDev State is not yet DEV_ZB_COORD the ZC hasn't restored or formed a network.
  			if ( newDevState != DEV_ZB_COORD )
  			{
     			  newDevState = DEV_COORD_STARTING;

  			  // If the notification status is either NETWORK RESTORED (with commMode INITIALIZATION) or
  			  // its SUCCESS (with FORMATION). Both indicate that the network is now ready.
  			  if(((appCnfMsg->status == COMM_NOTI_NETWORK_RESTORED)
  				&&(appCnfMsg->commissioningMode == COMM_MODE_INITIALIZATION))
  				||((appCnfMsg->status == COMM_NOTI_SUCCESS)
  				&&(appCnfMsg->commissioningMode == COMM_MODE_FORMATION)))
  				{
  				  //Set newDevState to DEV_ZB_COORD
  				  newDevState = DEV_ZB_COORD;

  				  //If the flag zspbPermitJoin is TRUE send Network steering.
  				  if ( zspbPermitJoin ){
  				  	ZDP_AppCnfStartCommissioningReq(MODE_NWK_STEERING);
  				  }
  				}
  			    // If the notification status wasn't NETWORK RESTORED or SUCCESS, but it was IN PROGRESS.
  			    else if(appCnfMsg->status == COMM_NOTI_IN_PROGRESS)
  				{
  				  //Set the newDevState to DEV_COORD_STARTING. 
  				  //This indicates that the ZC is working on Restoring or Formation of a new NWK, but it's not quite ready.
  				  newDevState = DEV_COORD_STARTING;
  				}
  		
  			}
			  //Send the payload parameters of the Notification to be processed and sent to NWK Layer
			  processNwkCommissioningNoti( appCnfMsg->status, appCnfMsg->commissioningMode, appCnfMsg->remainingCommMode);
			}
		  break;

		case GP_INCOMING_SEC_RSP_CMD:
		  processGpSecReqMsgInd( (gp_SecReq_t *) MSGpkt );
		  break;

		case GP_INCOMING_DATA_IND_CMD:
		  processGpDataIndMsgInd( (gp_DataInd_t *) MSGpkt );
		  break;

        default:
			uiPrintfEx(trERROR,"INVALID EVENT");
          break;
      }
                     ...
                     uint8 *pData, uint8 type )
{
  ...
  {
    //Don't talk to the API client if mutex couldn't be created
    if (!(zstack_mutex_valid)) return;
    ...
    switch ( cmdId )
    {
      ...
      case ZSTACK_CMD_IDS__APP_CNF_START_COMMISSIONING_REQ:
        {
          AppCnfStartCommissioningReq *pReq;
          pReq = app_cnf_start_commissioning_req__unpack( NULL, len, pData );
          if ( pReq )
          {
            processAppCnfStartCommissioningReq( connection, pReq );
            app_cnf_start_commissioning_req__free_unpacked( pReq, NULL );
          }
        }
        break;
      case ZSTACK_CMD_IDS__GP_SEC_RSP_MSG:
        {
          GpSecRspMsg *pGpSecRspMsg;
          pGpSecRspMsg = gp_sec_rsp_msg__unpack( NULL, len, pData );
          if ( pGpSecRspMsg )
          {
            sendGpSecRsp( pGpSecRspMsg );
            gp_sec_rsp_msg__free_unpacked( pGpSecRspMsg, NULL );
          }
        }
        break;

	  case ZSTACK_CMD_IDS__APP_CNF_ADD_INSTALL_CODE_REQ:
	  	{
          AppCnfAddInstallCodeReq *pReq;
          pReq = app_cnf_add_install_code_req__unpack( NULL, len, pData );
          if ( pReq )
          {
            processAppCnfAddInstallCodeReq( connection, pReq );
            app_cnf_add_install_code_req__free_unpacked( pReq, NULL );
          }
        }
        break;

	  case ZSTACK_CMD_IDS__ZCL_ACTION_REQ:
        {
          ZclActionReq *pZclActionReq;
          pZclActionReq = zcl_action_req__unpack( NULL, len, pData );
          if ( pZclActionReq )
          {
            processZclActionReq( connection, pZclActionReq );
            zcl_action_req__free_unpacked( pZclActionReq, NULL );
          }
        }
        break;
		
      case ZSTACK_CMD_IDS__UTIL_GET_DEV_NWK_INFO_REQ:
        {
          nwkIB_t NIB;
          // Get NIB nwk info from ZNP
          GetDevNwkInfo( &NIB );
          // Send nwk info to gw server
          SendDevNwkInfo( &NIB );
        }
        break;
      case ZSTACK_CMD_IDS__UTIL_SET_DEV_NWK_INFO_REQ:
        {
		  UtilSetDevNwkInfoReq *pInMsg;
		  pInMsg = util_set_dev_nwk_info_req__unpack(NULL, len, pData);
          if ( pInMsg )
          {
			processSetDevNwkInfo( connection, pInMsg );
			util_set_dev_nwk_info_req__free_unpacked(pInMsg, NULL);
          }
        }
        break;
      case ZSTACK_CMD_IDS__GP_ADDRESS_CONFLICT:
        {
          AFAddr *pInMsg;
          pInMsg = afaddr__unpack(NULL, len, pData);
          if ( pInMsg )
          {
            processGpAddressConflict( connection, pInMsg );
            afaddr__free_unpacked(pInMsg, NULL);
          }
        }
        break;
    }
    ...
    case Device_annce:
      {
		...
		sendDeviceAnnceShadowGw( inMsg->srcAddr.addr.shortAddr, &devAnn );
      }
      break;
 ...
 *
 * @brief   process incoming APP CNF Commissioning Notification
 *
 * @param   state - status
 *
 * @return  none
...
static void processNwkCommissioningNoti( uint8 status, uint8 commMode, uint8 remainingCommMode )
{
  ...
  int len;
  uint8 *pBuf;
  AppCnfCommissioningNotification commNoti = APP_CNF_COMMISSIONING_NOTIFICATION__INIT;
  
  uiPrintfEx(trINFO, "zstackpb: AppCnfCommissioningNotification: state:%d\n", status );
  uiPrintfEx(trINFO, "zstackpb: AppCnfCommissioningNotification: commMode:%d\n", commMode );
  uiPrintfEx(trINFO, "zstackpb: AppCnfCommissioningNotification: remainingCommMode:%d\n", remainingCommMode );
  
  commNoti.cmdid = ZSTACK_CMD_IDS__APP_CNF_COMMISSIONING_NOTIFICATION;
  commNoti.status = status;
  commNoti.status = commMode;
  commNoti.status = remainingCommMode;
  
  len = app_cnf_commissioning_notification__get_packed_size( &commNoti );
  pBuf = malloc( len );
  if ( pBuf )
  {
    app_cnf_commissioning_notification__pack( &commNoti, pBuf );
  
    sendMsgToAllCBMsgs( len, pBuf, ZS_APP_CNF_COMMISSIONING_NOTI_CBID,
 	  ZSTACK_CMD_IDS__APP_CNF_COMMISSIONING_NOTIFICATION );
...
static void processGpSecReqMsgInd( gp_SecReq_t *pkt )
{
  int len;
  uint8 *pBuf;
  GpSecReqIncomingMsgInd inMsg = GP_SEC_REQ_INCOMING_MSG_IND__INIT;

  inMsg.cmdid = ZSTACK_CMD_IDS__GP_SEC_REQ_INCOMING_MSG;

  inMsg.appid = pkt->gpd_ID.AppID;
  if( inMsg.appid == GP_APP_ID_DEFAULT )
  {
    inMsg.srcid = pkt->gpd_ID.GPDId.SrcID;
  }
  else if( inMsg.appid == GP_APP_ID_GP )
  {
    memcpy( &inMsg.gpdextaddr, pkt->gpd_ID.GPDId.GPDExtAddr, 8 );
    inMsg.endpoint = pkt->EndPoint;
  }

  inMsg.gpdfseclvl = pkt->gp_SecData.GPDFSecLvl;
  inMsg.gpdfkeytype = pkt->gp_SecData.GPDFKeyType;
  inMsg.gpdsecframecounter = pkt->gp_SecData.GPDSecFrameCounter;
  inMsg.dgpstubhandle = pkt->dGPStubHandle;

  len = gp_sec_req_incoming_msg_ind__get_packed_size( &inMsg );
  pBuf = malloc( len );
  ...
  if ( pBuf )
  {
    ...
    gp_sec_req_incoming_msg_ind__pack( &inMsg, pBuf );

    pItem = epTableFindEntryEP( 0xF2 );
    if ( pItem )
    {
      APIS_SendData( pItem->connection, ZSTACK_SYS_IDS__RPC_SYS_PROTOBUF, TRUE,
          ZSTACK_CMD_IDS__GP_SEC_REQ_INCOMING_MSG, len, pBuf );
    }
...
static void processGpDataIndMsgInd( gp_DataInd_t *pkt )
{
  int len;
  uint8 *pBuf;
  uint8 invalidAddr[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
  GpDataIndIncomingMsgInd inMsg = GP_DATA_IND_INCOMING_MSG_IND__INIT;

  inMsg.cmdid = ZSTACK_CMD_IDS__GP_DATA_IND_INCOMING_MSG;

  inMsg.status = pkt->status;
  inMsg.rssi = pkt->Rssi;
  inMsg.linkquality = pkt->LinkQuality;
  inMsg.seqnumber = pkt->SeqNumber;
  inMsg.addrmode = pkt->srcAddr.addrMode;
  inMsg.srcpanid = pkt->srcPanID;

  if( inMsg.addrmode == SADDR_MODE_SHORT )
  {
    inMsg.shortaddr = pkt->srcAddr.addr.shortAddr;
    memcpy( &inMsg.extaddr, invalidAddr, 8 );
  }
  else if( inMsg.addrmode == SADDR_MODE_EXT )
  {
    inMsg.shortaddr = 0xFFFF;
    memcpy( &inMsg.extaddr, pkt->srcAddr.addr.extAddr, 8 );
  }

  inMsg.appid = pkt->appID;
  inMsg.gpdfseclvl = pkt->GPDFSecLvl;
  inMsg.gpdfkeytype = pkt->GPDFKeyType;
  inMsg.autocommissioning = pkt->AutoCommissioning;
  inMsg.rxaftertx = pkt->RxAfterTx;
  inMsg.srcid = pkt->SrcId;
  inMsg.endpoint = pkt->EndPoint;
  inMsg.gpdsecframecounter = pkt->GPDSecFrameCounter;
  inMsg.gpdcmmdid = pkt->GPDCmmdID;
  inMsg.mic = pkt->MIC;
  inMsg.gpdasdu.len = pkt->GPDasduLength;
  inMsg.gpdasdu.data = malloc( pkt->GPDasduLength );
  if ( inMsg.gpdasdu.data )
  {
    memcpy( inMsg.gpdasdu.data, pkt->GPDasdu, pkt->GPDasduLength );
  }

  len = gp_data_ind_incoming_msg_ind__get_packed_size( &inMsg );
  pBuf = malloc( len );

  if ( pBuf )
  {
    epItem_t *pItem;
    gp_data_ind_incoming_msg_ind__pack( &inMsg, pBuf );

    pItem = epTableFindEntryEP( 0xF2 );
    if ( pItem )
    {
      APIS_SendData( pItem->connection, ZSTACK_SYS_IDS__RPC_SYS_PROTOBUF, TRUE,
          ZSTACK_CMD_IDS__GP_DATA_IND_INCOMING_MSG, len, pBuf );
    }
...
static void sendDefaultRsp( int connection, uint8 cmdId, ZStatusValues status )
{
  ...
  if ( pBuf )
  {
    ...
    APIS_SendData( connection, ZSTACK_SYS_IDS__RPC_SYS_PROTOBUF, FALSE, ZSTACK_CMD_IDS__ZSTACK_DEFAULT_RSP,
        len, pBuf );
...
static void processZdoMgmtBindReq( int connection, ZdoMgmtBindReq *pMsg )
{
  ...
  nwkAddress = (uint16) pMsg->nwkaddr;
  startIndex = (uint8) pMsg->startindex;

  status = (uint8) ZDP_MgmtBindReq( nwkAddress, startIndex, TRUE );
...
/*********************************************************************
 * @fn      processAppCnfStartCommissioningReq
 *
 * @brief   Process incoming App Config BDB Start Commissioning Request.
 *
 * @param   connection - connection handle (tcp)
 * @param   pMsg - pointer to Request
 *
                                         ...
                                         AppCnfStartCommissioningReq*pMsg )
{
  ...
  status = (uint8) ZDP_AppCnfStartCommissioningReq( (uint8) pMsg->commissioningmode);

  sendDefaultRsp( connection, pMsg->cmdid, (ZStatusValues) status );
}

/*********************************************************************
 * @fn      processAppCnfAddInstallCodeReq
 *
 * @brief   Process incoming App Config BDB Start Commissioning Request.
 *
 * @param   connection - connection handle (tcp)
                                         ...
                                         AppCnfAddInstallCodeReq*pMsg )
{
  uint8 status;
  uint8 ieeeAddr[8];
  uint8 installCodeValue[16];
  
  memcpy( ieeeAddr, &(pMsg->ieeeaddress), 8 );
  memcpy( installCodeValue, pMsg->installcode.data, pMsg->installcode.len);

  status = (uint8) ZDP_AppCnfAddInstallCodeReq( ieeeAddr, pMsg->installcode.len, installCodeValue, (uint16)pMsg->crc);

  sendDefaultRsp( connection, pMsg->cmdid, (ZStatusValues) status );
}

/*********************************************************************
 * @fn		processZclActionReq
 *
 * @brief	Process incoming ZCL Action Request.
 *
 * @param	connection - connection handle (tcp)
 * @param	pMsg - pointer to Request
 *
										 ...
										 ZclActionReq *pMsg )
{
  int i;
  uint8 status;
  uint16 cmdID = pMsg->action;
  uint16 id = pMsg->id;
  uint16 ndx = pMsg->index;
  uint16 len = pMsg->payload.len;
  uint8 pData[len];
  uint8 *pBuf;
  uint8 pbLength;

  ZclActionRsp niRsp = ZCL_ACTION_RSP__INIT;
  niRsp.cmdid = ZSTACK_CMD_IDS__ZCL_ACTION_RSP;
  
  memcpy( pData, pMsg->payload.data, len );

  // TODO: Serialize init messages on GP initialization MNH: TEST WITHOUT THE FOR AND LEAVE pDATA as it is (not initialized)
  for (i=0;i<65;i++)
  {
	//NO-OP Cool down for the previous message to reach 
  }

  status = (uint8) ZDP_ZclActionReq( cmdID, id, ndx, len, (void *)pData );
  
  niRsp.action = cmdID;
  niRsp.id = id;
  niRsp.index = status;
  niRsp.payload.len = len;

  niRsp.payload.data = malloc(len);

  if (niRsp.payload.data)
  {
  	memcpy( niRsp.payload.data, pData, len );

    pbLength = zcl_action_rsp__get_packed_size( &niRsp );
    
    pBuf = malloc( pbLength );
    if ( pBuf )
    {
      zcl_action_rsp__pack( &niRsp, pBuf );
  	
      // Send the AREQ message
      APIS_SendData( connection, ZSTACK_SYS_IDS__RPC_SYS_PROTOBUF, FALSE,
          ZSTACK_CMD_IDS__ZCL_ACTION_RSP, pbLength, pBuf );
  	
      free( pBuf );
    }

  	free( niRsp.payload.data );
  }
  //Critical ERROR
...
static void sendMsgToAllCBs( int len, uint8 *pBuf, uint16 cbMask, uint8 cmdId )
{
  ...
  pthread_mutex_lock(&zstack_mutex);
  srch = pEpTableHdr;
  ...
  pthread_mutex_unlock(&zstack_mutex);
}
...
/*********************************************************************
 * @fn          APPCNFCommissioningNotiCB
 *
 * @brief       callback function to handle Commissioning Notification
 *
 * @param       pStr - pointer to Commissioning Notification Information
 *
 * @return      NULL
 */
void *APPCNFCommissioningNotiCB( void *pStr )
{
  ...
  uint8 *pBuf;
  AppCnfCommissioningNotification commNoti = APP_CNF_COMMISSIONING_NOTIFICATION__INIT;
  NLME_CommissioningNoti_t *pCommissioningNoti = (NLME_CommissioningNoti_t *) pStr;

  uiPrintfEx(trINFO, "zstackpb Received APP CNF Commissioning Notification\n" );

  commNoti.cmdid = ZSTACK_CMD_IDS__APP_CNF_COMMISSIONING_NOTIFICATION;
  commNoti.status = pCommissioningNoti->status;
  commNoti.commmode = pCommissioningNoti->commissioningMode;
  commNoti.remainingcommmode= pCommissioningNoti->remainingCommMode;
  ...
  {
    app_cnf_commissioning_notification__pack( &commNoti, pBuf );
    sendMsgToAllCBs( len, pBuf, ZS_APP_CNF_COMMISSIONING_NOTI_CBID,
        ZSTACK_CMD_IDS__APP_CNF_COMMISSIONING_NOTIFICATION );
...
void sendDeviceAnnceShadowGw( uint16 srcAddr, ZDO_DeviceAnnce_t *pDevAnn )
{
  ...
  uint8 *pBuf;
  CapabilityInfo capInfo = CAPABILITY_INFO__INIT;
  ZdoDeviceAnnounceShadowInd devInd = ZDO_DEVICE_ANNOUNCE_SHADOW_IND__INIT;

  uiPrintfEx(trINFO, "zstackpb Sending Device Announce Ind to Gateway Server\n" );

  devInd.cmdid = ZSTACK_CMD_IDS__ZDO_DEVICE_ANNOUNCE_SHADOW;
  devInd.srcaddr = srcAddr;
  devInd.devaddr = pDevAnn->nwkAddr;
  ...
  len = zdo_device_announce_shadow_ind__get_packed_size( &devInd );
  pBuf = malloc( len );
  if ( pBuf )
  {
    zdo_device_announce_shadow_ind__pack( &devInd, pBuf );

	pItem = epTableFindEntryEP( 0xF2 );
	if ( pItem )
	{
	  APIS_SendData( pItem->connection, ZSTACK_SYS_IDS__RPC_SYS_PROTOBUF, TRUE,
		  ZSTACK_CMD_IDS__ZDO_DEVICE_ANNOUNCE_SHADOW, len, pBuf );
	}
...
static void SendDevNwkInfo( nwkIB_t* pNIB )
{
  int len;
  uint8 *pBuf;
  SendGetDevNwkInfoReq NIB = SEND_GET_DEV_NWK_INFO_REQ__INIT;

  NIB.nwkdevaddress = pNIB->nwkDevAddress;
  NIB.nwkpanid = pNIB->nwkPanId;
  NIB.sequencenum = pNIB->SequenceNum;
  NIB.nwklogicalchannel = pNIB->nwkLogicalChannel;

  len = send_get_dev_nwk_info_req__get_packed_size( &NIB );
  pBuf = malloc( len );
  if ( pBuf )
  {
    epItem_t *pItem;

    send_get_dev_nwk_info_req__pack( &NIB, pBuf );

    pItem = epTableFindEntryEP( 0xF2 );
    if ( pItem )
    {
      APIS_SendData( pItem->connection, ZSTACK_SYS_IDS__RPC_SYS_PROTOBUF, FALSE,
          ZSTACK_CMD_IDS__SEND_GET_DEV_NWK_INFO_REQ, len, pBuf );
    }
...
static void processSetDevNwkInfo( int connection, UtilSetDevNwkInfoReq* pMsg )
{
  ...
  if(pMsg)
  {
    *pBuf++ = LO_UINT16(pMsg->nwkdevaddress);
    *pBuf++ = HI_UINT16(pMsg->nwkdevaddress);
    *pBuf++ = LO_UINT16(pMsg->nwkpanid);
    *pBuf++ = HI_UINT16(pMsg->nwkpanid);
    *pBuf++ = pMsg->sequencenum;
    *pBuf++ = pMsg->nwklogicalchannel;

    // send request to NPI synchronously
    status = (uint8)SYS_SetDevNwkInfoReq( 6, buffer );
...
static void processGpAddressConflict( int connection, AFAddr* pMsg )
{
  ...
  if(pMsg)
  {
    *pBuf++ = LO_UINT16(pMsg->shortaddr);
    *pBuf++ = HI_UINT16(pMsg->shortaddr);

    // send request to NPI synchronously
    status = (uint8)SYS_GpAddressConflict( 2, buffer );

    sendDefaultRsp( connection, ZSTACK_CMD_IDS__GP_ADDRESS_CONFLICT, (ZStatusValues) status );
  }
}
